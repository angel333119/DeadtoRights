...
namespace DeadtoRights
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog1.Filter = "Dead to Rights|*.GBA|Todos os arquivos (*.*)|*.*";
            openFileDialog1.Title = "Abrir ROM Dead to Rights...";
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (String file in openFileDialog1.FileNames)
                {
...

                    {
                        ...
                        if (magic == 0x44414544) // Magic do arquivo = DEAD
                        {
                            game = 0x54F0D4;   //primeiro ponteiro do jogo - OBS: faço assim por ser mais fácil pra mim
                            ppgame = 0x54F0D8; //ppgame = proximo ponteiro do game
                        }
                        else
                        {
                            MessageBox.Show("O arquivo não é uma rom do jogo Dead to Rights!", "AVISO!");
                            return;
                        }

                        br.BaseStream.Seek(0x54F0D0, SeekOrigin.Begin);
                        ...
                        br.BaseStream.Seek(0x54F33B, SeekOrigin.Begin);

                        verificador = br.ReadByte();
                        ...
                        for (int i = 0; i < totaldetextos; i++)
                        {
                            ...
                            while (acabouotexto == false)
                            {
                                ...
                                if (comparador == 0x00)
                                {
                                    ...
                                    br.BaseStream.Seek(ponteiro + 0x54F0D0, SeekOrigin.Begin);
                                }
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                using (FileStream stream = File.Open(dump, FileMode.Open))
                {
                    ...
                    if (file.Exists)
                    {
                        ...
                        br.BaseStream.Seek(0x54F0D4, SeekOrigin.Begin);
                        ...
                        {
                            bw.BaseStream.Seek(0x54F0D4 + pp, SeekOrigin.Begin);
                            ...
                            {
                                bw.BaseStream.Seek(ponteiro + 0x54F0D0, SeekOrigin.Begin);
